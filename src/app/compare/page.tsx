...
export default async function ComparePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [options, release, left, right] = await Promise.all([listBeerOptions(), getLatestRelease(), leftSlug ? getBeerBySlug(leftSlug) : Promise.resolve(null), rightSlug ? getBeerBySlug(rightSlug) : Promise.resolve(null)]);
      ...
      {sameBeer ? <div className="empty-state"><h2>Choose two different beers</h2><p>Keep either selection, then choose another beer for the other side.</p></div>
        ...
        : left && right ? <>
          ...
          <section className="compare-subjects" aria-label="Selected beers">
            {[left, right].map((beer, index) => <article className="compare-subject" key={beer.id}><div><span>{index === 0 ? "First beer" : "Second beer"}</span><h2><Link href={`/beers/${beer.slug}`}>{beer.name}</Link></h2><p>{beer.brewery_name} · {beer.country_name}</p><Link href={`/styles/${beer.style_slug}`}>{beer.style_name}</Link></div><strong>{beer.index_score.toFixed(1)}</strong></article>)}
          </section>
