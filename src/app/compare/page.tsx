  ...
  return (
    <div className="container page compare-page">
      ...
      <header className="page-header compare-header">
        <div>
          <h1>Compare two pours</h1>
          <p>See strength, bitterness, body, and finish side by side. The differences help you choose; they do not declare a universal winner.</p>
        </div>
        {complete && left && right ? (
          <ExportButton
            href={`/api/export/compare?a=${left.slug}&b=${right.slug}`}
        ...
        : left && right ? <>
          <section className="compare-subjects" aria-label="Selected beers">
            {[left, right].map((beer, index) => (
              <article
                aria-label={`${index === 0 ? "First" : "Second"} beer: ${beer.name}`}
                className="compare-subject"
                key={beer.id}
              >
                <div>
                  <h2><Link href={`/beers/${beer.slug}`}>{beer.name}</Link></h2>
                  <p className="country-inline">{beer.brewery_name} · <CountryFlag isoCode={beer.iso_code} />{beer.country_name}</p>
